...
class MyCrawler:
    ...
    def getImages(cls, content, path):
        ...
        if imageList is not None:
            ...
            for imageUrl in newList:
                # 打开网址，下载图片保存到本地
                urllib.request.urlretrieve(imageUrl, '{}{}.jpg'.format(path, MyCrawler.imgCount))
                MyCrawler.imgCount += 1  # 计数器
    ...
    def extract_a_label(cls, content, path):
        ...
        # 写入文件 按编号写入
        childList = []
        file_ob = open(path + 'childUrl' + str(MyCrawler.urlCount) + '.txt', 'w', encoding='utf-8')
        ...
        file_ob.close()
        print(Color.yellow + "# childLink " + str(MyCrawler.urlCount) + " Writing Successfully...")
    ...
    def cuteCrawler(cls, url):
        ...
        """
        dic = {}  # 字典
        id = MyCrawler.urlCount  # 编号记录
        # 获取网页内容
        try:
            # 无法访问 这一句报错
            page = urllib.request.urlopen(url)
            # 接下来都是正常访问的情况
            MyCrawler.urlCount += 1  # 网址编号++
            ...
            # 获取图片 存储位置 : Image
            cls.getImages(content, 'BFS/Image/')
            # 获取html正文
    ...
    def BFS_GetInfo(cls, ceedUrl, quantity):
        ...
        while urlQueue:
            ...
            bf.insert(url)  # 计入过滤器
            childList = MyCrawler.cuteCrawler(url)
            print(Color.carmine + '# 已爬取网站数:', MyCrawler.urlCount, ' 图片数:', MyCrawler.imgCount)
            # 退出循环
            if MyCrawler.urlCount >= quantity:
                break
            # 筛选网址
            # print(childList)
            newChildList = []
...
if __name__ == '__main__':
    ...
    MyCrawler.BFS_GetInfo(url, quantity)  # 广度遍历
    pass
