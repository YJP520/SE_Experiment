...
class PRIterator:
    ...
    def __init__(self, filePath, d=0.5, count=100, delta=0.00001):
        """
        PageRank 构造函数
        :param filePath: 文件地址
        """
        self.graph = self._readData(filePath)  # 要计算的图
...
if __name__ == '__main__':
    ...
    pr = PRIterator(path, d=0.5, count=100, delta=0.00001)
    page_ranks = pr.pageRank()
    print(Color.green, "# The final page rank is", page_ranks)
    # 排序 按键值逆序排列
    # page_ranks = dict(sorted(page_ranks.items(), key=lambda e: e[0], reverse=False))  # key
    page_ranks = dict(sorted(page_ranks.items(), key=lambda e: e[1], reverse=True))  # value
    print(Color.green, "# Sorted rank is", page_ranks)
    pass
